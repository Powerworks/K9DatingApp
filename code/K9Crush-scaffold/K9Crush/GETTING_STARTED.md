...
This brings up RabbitMQ, Redis, smtp4dev, the Grafana LGTM-in-one-container image, and — as of this writing, a **deliberate, temporary reversal of ADR-024** for active local build-out — a local Postgres container too (`appsettings.Development.json`'s `ConnectionStrings:Postgres` already points at it, `k9crush`/`k9crush`/`localhost:5432`). Storage and Auth are still Supabase-managed, set up in Step 2 below; only Postgres moved local for now. Give it a minute — check everything's healthy:
