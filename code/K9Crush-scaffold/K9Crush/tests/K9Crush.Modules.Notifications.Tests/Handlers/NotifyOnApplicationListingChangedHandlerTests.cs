using Marten;
using NSubstitute;
using K9Crush.Modules.Notifications.Api.Automations.NotifyOnApplicationListingChanged;
...
/// <summary>
/// Layer 2 (TestingApproach.md) - NotifyOnApplicationListingChangedHandler
...
/// (ADR-031).
/// </summary>
public class NotifyOnApplicationListingChangedHandlerTests
{
    private static ApplicationListingChangedV1 BuildEvent(Guid applicantOwnerId) => new(
    ...
    public async Task Handle_WhenApplicantHasAKnownEmailAndDefaultPreferences_SendsAndLogsEmail()
    {
        ...
        await NotifyOnApplicationListingChangedHandler.Handle(BuildEvent(applicantOwnerId), session, sender, CancellationToken.None);
    ...
    {
        var applicantOwnerId = Guid.NewGuid();
        var (preference, _) = NotificationPreference.CreateDefaultNew(applicantOwnerId);
