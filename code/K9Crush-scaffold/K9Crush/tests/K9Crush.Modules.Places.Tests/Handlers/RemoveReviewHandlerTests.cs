...
using NSubstitute;
using K9Crush.Modules.Places.Api.Commands.RemoveReview;
...
/// <summary>
/// Layer 2 (TestingApproach.md) - RemoveReviewHandler only calls
...
public class RemoveReviewHandlerTests
{
    ...
    public async Task Handle_WhenReviewDoesNotExist_ReturnsNotFound()
    {
        ...
        var result = await RemoveReviewHandler.Handle(reviewId, BuildUser(ReviewerOwnerId), session, CancellationToken.None);
    ...
    public async Task Handle_WhenCallerIsNotTheReviewer_ReturnsForbid()
    {
        ...
        var result = await RemoveReviewHandler.Handle(review.Id, BuildUser(Guid.NewGuid()), session, CancellationToken.None);
    ...
    public async Task Handle_WhenNotPublished_ReturnsConflict()
    {
        ...
        var result = await RemoveReviewHandler.Handle(review.Id, BuildUser(ReviewerOwnerId), session, CancellationToken.None);

        result.Result.Should().BeOfType<Conflict<string>>();
    }

    [Fact]
    public async Task Handle_WhenPublishedAndCallerIsReviewer_RemovesAndPersists()
    {
        var review = Review.Write(Guid.NewGuid(), ReviewerOwnerId, 5, "Great walk!", false);
        review.Publish();
        var session = Substitute.For<IDocumentSession>();
