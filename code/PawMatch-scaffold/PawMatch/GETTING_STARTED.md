...
- **Dockerfiles exist now** (`deploy/docker/*.Dockerfile`) but are unbuilt/untested this session - same caveat as everything else in this scaffold. Build them yourself before trusting them:
  ```bash
  docker build -f deploy/docker/ApiHost.Dockerfile -t pawmatch-api-host .
  docker build -f deploy/docker/Gateway.Dockerfile -t pawmatch-gateway .
  docker build -f deploy/docker/BlazorApp.Dockerfile -t pawmatch-blazor-app .
  ```
