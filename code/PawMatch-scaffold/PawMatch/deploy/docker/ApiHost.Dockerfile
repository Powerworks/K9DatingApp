# Build from the repo root: docker build -f deploy/docker/ApiHost.Dockerfile -t pawmatch-api-host .
FROM mcr.microsoft.com/dotnet/sdk:10.0 AS build
WORKDIR /src

COPY . .

RUN dotnet restore src/Host/PawMatch.Api.Host/PawMatch.Api.Host.csproj
RUN dotnet publish src/Host/PawMatch.Api.Host/PawMatch.Api.Host.csproj \
...
ENTRYPOINT ["dotnet", "PawMatch.Api.Host.dll"]
