# Build from the repo root: docker build -f deploy/docker/Gateway.Dockerfile -t pawmatch-gateway .
...
RUN dotnet restore src/Gateway/PawMatch.Gateway/PawMatch.Gateway.csproj
RUN dotnet publish src/Gateway/PawMatch.Gateway/PawMatch.Gateway.csproj \
...
ENTRYPOINT ["dotnet", "PawMatch.Gateway.dll"]
