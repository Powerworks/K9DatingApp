...
└── <Module>Module.cs
```
This replaced the flatter `Features/` folder from the first pass of the scaffold. `PawMatch.Modules.Profiles.Api`, `PawMatch.Modules.Discovery.Api`, `PawMatch.Modules.Identity.Api`, and `PawMatch.Modules.ShelterAdoption.Api` are organized this way.
