...
**Where:** `tests/PawMatch.Modules.<Module>.Tests/Domain/`, one test class
per entity, mirroring `src/Modules/<Module>/PawMatch.Modules.<Module>.Domain/`.
...
**Where:** `tests/PawMatch.Modules.<Module>.Tests/Handlers/`, one test class
...
**Where:** `tests/PawMatch.IntegrationTests/`, organized by module, not
...
project existed; `PawMatch.ArchitectureTests` now implements the two that
...
1. **Module boundary rule** (NetArchTest): every module's `.Domain` assembly
   must depend on `PawMatch.BuildingBlocks.Domain` only, never another
...
```
tests/
  PawMatch.ArchitectureTests/            (Layer 4, solution-wide)
  PawMatch.Modules.ShelterAdoption.Tests/  (Layers 1–2, one project per module)
    Domain/
    Handlers/
  PawMatch.IntegrationTests/              (Layer 3, cross-module, Testcontainers)
```
