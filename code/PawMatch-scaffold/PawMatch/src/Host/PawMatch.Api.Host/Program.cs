...
using PawMatch.Modules.Discovery.Api;
using PawMatch.Modules.Identity.Api;
using PawMatch.Modules.Profiles.Api;
...
.IntegrateWithWolverine(m =>
{
    ...
    // See PawMatch.Modules.Discovery.Api.Automations.DetectMutualMatch
    ...
    m.SubscribeToEvent<PawMatch.Modules.Discovery.Domain.Events.DogLiked>();
});
...
builder.Host.UseWolverine(opts =>
{
    ...
    opts.PublishAllMessages().ToRabbitExchange("pawmatch.events");
    ...
    // pawmatch.events and dropped - confirmed live (see IModule.cs's
    ...
    foreach (var module in modules)
    {
        ...
        {
            opts.ListenToRabbitQueue(queueName, queue => queue.BindExchange("pawmatch.events"));
        }
    }
