...
This repo keeps the shared config at the **repo root** (`<repo-root>/.eventmodelers/config.json`), which the walk-up above finds regardless of which directory (e.g. `build-kit-dotnet/`, `code/K9Crush-scaffold/K9Crush/`) the session's cwd happens to be in, as long as it's inside the repo.
